...
All commands use `~/.claude/skills/chrome-cdp/~/.claude/skills/chrome-cdp/scripts/cdp.mjs`. The `<target>` is a **unique** targetId prefix from `list`; copy the full prefix shown in the `list` output (for example `6BE827FA`). The CLI rejects ambiguous prefixes.
